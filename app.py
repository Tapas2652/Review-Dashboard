...
uploaded = st.sidebar.file_uploader("Upload / replace raw data workbook", type=["xlsx"])
if uploaded:
    with open(default_path, "wb") as f:
        f.write(uploaded.getbuffer())
    st.sidebar.success("File updated — dashboard refreshed.")

if not os.path.exists(default_path):
    st.error(f"Place `{default_path}` next to this script, or upload it from the sidebar.")
    st.stop()

data = dl.get_data(default_path)
