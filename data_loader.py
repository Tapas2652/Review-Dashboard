...
Reads directly from the raw Excel workbook (Review_Data.xlsx) — every time
the underlying file changes on disk, Streamlit's cache key (file mtime)
changes too, so the dashboard picks up the new data automatically on
the next rerun / refresh.
"""
import os
import re
...
def load_all(path: str, _mtime: float):
    """Load and lightly clean every sheet we need. `_mtime` is only used
    as a cache-busting key so edits to the source file are picked up."""
...
def get_data(path="Review_Data.xlsx"):
    mtime = os.path.getmtime(path)
    return load_all(path, mtime)
